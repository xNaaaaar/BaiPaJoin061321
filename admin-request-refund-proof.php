  ...
  if(!isset($_SESSION['admin'])) header("Location: login.php");
	...
	if(isset($_POST['btnUpload'])){
		...
		if($imageName === 1){
			...
			DB::query('UPDATE request SET req_img=?, req_status=?, req_rcvd=? WHERE req_id=?', array($imageName, "refunded", 2, $_GET['req_id']), "CREATE");

			## EMAIL + SMS NOTIFICATION
			## FOR JOINER NANI LEX, GI COMMENT LANG NAKO SA UBOS KAY FOR SURE SA ORGANIZER NI OG KAY GI COPY RASAD NAKO NGA PAGE SA admin-request-payout-proof.php
			...
			header("Location: admin-request-refund.php?success");
		}
	}
?>
<!-- Head -->
      ...
      <div class="main_con">
        ...
        <main>
          ...
          <div class="contents">
            <div class="admins">
							<?php
							$details = DB::query("SELECT * FROM request r JOIN booking b ON r.book_id=b.book_id JOIN adventure a ON b.adv_id=a.adv_id JOIN joiner j ON j.joiner_id=b.joiner_id WHERE req_id=?", array($_GET['req_id']), "READ");
							$details = $details[0];
							?>
							<section>
								<h3><?php echo $details['joiner_fname']; ?>'s Adventure Details</h3>
								<table>
									<tr>
										<td>Adventure ID</td>
									...
									<tr>
										<td>Booked Guests</td>
										<td>: <b><?php echo $details['book_guests']; ?></b></td>
									</tr>
									<tr>
										<td>Total Amount</td>
							...
							<form method="post" enctype="multipart/form-data">
								...
								<a class='edit' href="admin-request-refund.php">Back</a>
							</form>
            </div>
          </div>
        </main>
